...
namespace Symbolism.Tests
{
	...
	{
		private void Run(MathObject obj, string expected)
		{
		...
		{
			Run(sin(x)*cos(y), "sin(x) * cos(y)");
		}
		[TestMethod]
		public void MultiParameterFunction()
		{
			Run(and(x, y, z), "and(x, y, z)");
		}
		[TestMethod]
		...
		{
			Run(x ^ new Fraction(1, 3), "x ^ (1/3)");
		}
		[TestMethod]
		public void NestedAnd()
		{
			Run(and(and(x, y), and(x, z)), "and(and(x, y), and(x, z))");
		}
		[TestMethod]
		public void VeryComplex2()
		{
			Run(x == sqrt(2*(y*z - cos(a)*y*z)), "x == sqrt(2 * (y * z - cos(a) * y * z))");
		}
		[TestMethod]
